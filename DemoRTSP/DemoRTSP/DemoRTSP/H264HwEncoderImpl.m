                     ...
                     CMSampleBufferRef sampleBuffer )
{
    ...
    CVImageBufferRef imageBuffer4enc = (CVImageBufferRef)sourceFrameRefCon;
    ...
    //double pts = CMTimeGetSeconds(CMSampleBufferGetDecodeTimeStamp(sampleBuffer));
    //CMTime prestime = CMSampleBufferGetPresentationTimeStamp(sampleBuffer);
    //double pts = (double)(prestime.value) / prestime.timescale;
    ...
    if (isIFrame)
    {
        if (encoder->_delegate)
        {
            ...
            [encoder->_delegate inmemOnBeforeIframe:pts];
        }
    }
    if (isIFrame)
    {
    ...
    if (statusCodeRet == noErr) {
        ...
        while (bufferOffset < totalLength - AVCCHeaderLength) {
            ...
            [encoder->_delegate inmemEncodedVideoData:data withPts:pts isKeyFrame:isIFrame];
            
            // Move to the next NAL unit in the block buffer
            bufferOffset += AVCCHeaderLength + NALUnitLength;
        }
    }
    [encoder->_delegate inmemOnAfterEncodingVideoFrame:imageBuffer4enc];
    //NSLog(@"Frame: mixed %i %i %i, parsed %li of %li", isIFrame?1:0, isPps?1:0, isSps?1:0, bufferOffset, totalLength);
}

- (BOOL)setupEncoding
{
    if(self.audioSettingsMic == nil || self.videoSettingsOut == nil){
        return NO;
    }
    ...
    dispatch_sync(aQueue, ^{
        ...
        // Audio compression
        int bitrate = [[self.audioSettingsMic objectForKey:AVEncoderBitRateKey] intValue];//96000;
        int frequencyInHz = [[self.audioSettingsMic objectForKey:AVSampleRateKey] intValue];
        int channels = [[self.audioSettingsMic objectForKey:AVNumberOfChannelsKey] intValue];//2
        ...
        descAACFormat = a_out;
        self.audioSettingsOut = descAACFormat;
        ...
        if (result != noErr)
        {
            [AppDelegate SLog:(@"H264: Unable to create a PCM->AAC session")];
        ...
        int ww = [[self.videoSettingsOut objectForKey:AVVideoWidthKey] intValue];
        int hh = [[self.videoSettingsOut objectForKey:AVVideoHeightKey] intValue];
        frameW = ww;
        frameH = hh;
        // video compression
        ...
        if (result != noErr)
        {
            [AppDelegate SLog:(@"H264: Unable to create a H264 session")];
        ...
        // Set the properties
        const int framert = AP4_MUX_DEFAULT_VIDEO_FRAME_RATE;
        CFNumberRef ref = CFNumberCreate(NULL, kCFNumberSInt32Type, &framert);
        ...
        const int maxkfi = framert/2;
        CFNumberRef ref2 = CFNumberCreate(NULL, kCFNumberSInt32Type, &maxkfi);
        VTSessionSetProperty(vEncodingSession, kVTCompressionPropertyKey_MaxKeyFrameInterval, ref2);
        CFRelease(ref2);
...
- (void) encodeAudio:(CMSampleBufferRef)sampleBuffer
{
    ...
    dispatch_sync(aQueue, ^{
        ...
        if (statusCode != noErr) {
            [AppDelegate SLog:@"H264: AudioConverterFillComplexBuffer failed"];
            ...
            [encoder->_delegate inmemEncodedAudioData:fullData withPts:pts];
        }
        free(buffer);
...
- (void) encodeVideo:(CMSampleBufferRef)sampleBuffer
{
    ...
    dispatch_sync(aQueue, ^{
        ...
        CVImageBufferRef imageBufferInitial = (CVImageBufferRef)CMSampleBufferGetImageBuffer(sampleBuffer);
        CVImageBufferRef imageBuffer4enc = [self.delegate inmemOnBeforeEncodingVideoFrame:imageBufferInitial frameSize:CGSizeMake(frameW, frameH)];
        // Create properties
        ...
        OSStatus statusCode = VTCompressionSessionEncodeFrame(vEncodingSession,
                                                              ...
                                                              kCMTimeInvalid,
                                                              NULL, imageBuffer4enc, &flags);
        //if(imageBuffer != imageBufferInitial){
        //    CVPixelBufferRelease(imageBuffer);
        //}
        // Check for error
        if (statusCode != noErr) {
            [AppDelegate SLog:@"H264: VTCompressionSessionEncodeFrame failed"];
